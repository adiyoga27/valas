...
@section('title', 'Transaksi Jual Valas')
@section('heading', 'Buat Transaksi Penjualan Valas')
@section('content')
<div class="max-w-4xl" x-data="sellForm()">
    <form method="POST" action="{{ route('admin.sell-transactions.store') }}" class="space-y-6">
        @csrf
        <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-6 space-y-4">
            <h3 class="text-sm font-semibold text-gray-700">Invoice Penjualan</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label class="block text-xs font-medium text-gray-500 mb-1">No. Invoice</label><input type="text" name="transaction_code" x-model="transaction_code" readonly class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-500"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-xs font-medium text-gray-500 mb-1">Nama Pelanggan *</label><input type="text" name="customer_name" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500"></div>
                <div><label class="block text-xs font-medium text-gray-500 mb-1">Passport</label><input type="text" name="passport_number" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500"></div>
                <div><label class="block text-xs font-medium text-gray-500 mb-1">Alamat</label><input type="text" name="customer_address" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500"></div>
                <div><label class="block text-xs font-medium text-gray-500 mb-1">Negara</label><input type="text" name="customer_country" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500"></div>
            </div>
        </div>
                    ...
                    <div class="col-span-4 sm:col-span-3">
                        ...
                        <select x-model="item.currency_id" required :name="'items['+i+'][currency_id]'" class="w-full rounded-lg border border-gray-300 px-2 py-2 text-sm focus:border-amber-500 focus:ring-amber-500" x-on:change="onCurrencyChange(i)">
                            ...
                            @foreach($currencies as $c)
                                <option value="{{ $c->id }}" data-code="{{ $c->code }}" data-name="{{ $c->name }}" data-rate="{{ $c->sell_rate }}">{{ $c->code }} - {{ $c->name }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-6">
            ...
            <textarea name="notes" rows="2" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500"></textarea>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.sell-transactions.index') }}" class="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">Batal</a>
        </div>
    </form>
</div>
@push('scripts')
...
function sellForm() {
    return {
        ...
        transaction_code: '{{ \App\Models\SellTransaction::generateCode() }}',
        items: [{ currency_id: '', currency_code: '', currency_name: '', sell_rate: 0, qty: 1, total: 0 }],
        additionals: [],
        ...
        onCurrencyChange(i) {
            ...
            if (opt && opt.value) { this.items[i].currency_code = opt.dataset.code; this.items[i].currency_name = opt.dataset.name; this.items[i].sell_rate = parseFloat(opt.dataset.rate); this.recalcItem(i); }
        },
